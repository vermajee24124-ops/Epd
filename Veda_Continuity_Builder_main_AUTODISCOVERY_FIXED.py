...
def env_value(name: str, default: str) -> str:
    """Use default when a GitHub variable is missing OR passed as an empty string."""
...
def source_rows() -> List[Tuple[int, str]]:
    """
    First try HF_SOURCE_FOLDER.
    If the configured/default folder is wrong or empty, automatically discover
    a TXT episode series containing Episode_0001 ... Episode_0200 anywhere in
    the source dataset. This avoids a total 'Missing 1-200' failure caused only
    by a folder-name mismatch.
    """
    all_txt: List[Tuple[int, str]] = []
    ...
    for item in hf.list_repo_tree(
        ...
        ep = ep_no(path)
        if ep and 1 <= ep <= 200:
            all_txt.append((ep, path))

    preferred = [
        row for row in all_txt
        if row[1].startswith(HF_SOURCE_FOLDER.rstrip("/") + "/")
    ]
    preferred.sort(key=lambda x: x[0])

    def validate(rows: List[Tuple[int, str]]) -> bool:
        nums = [n for n, _ in rows]
        return nums == list(range(1, 201))

    if validate(preferred):
        print(f"PASS: using configured source folder: {HF_SOURCE_FOLDER}")
        return preferred

    print(
        f"WARNING: configured source folder '{HF_SOURCE_FOLDER}' did not contain "
        "an exact Episode 0001-0200 TXT sequence."
    )
    print("AUTO-DISCOVERY: scanning all TXT episode folders in the source dataset...")

    by_parent: Dict[str, List[Tuple[int, str]]] = {}
    for row in all_txt:
        parent = str(Path(row[1]).parent)
    ...
    for parent, rows in by_parent.items():
        ...
        if validate(rows):
            candidates.append((parent, rows))

    if len(candidates) == 1:
        parent, rows = candidates[0]
        print(f"AUTO-DISCOVERY PASS: exact 1-200 TXT sequence found in: {parent}")
        return rows

    if len(candidates) > 1:
        # Prefer folders whose names clearly look like transcript output.
        candidates.sort(
            key=lambda x: (
                "transcript" not in x[0].lower(),
                "txt" not in x[0].lower(),
                len(x[0]),
                x[0],
            )
        )
        parent, rows = candidates[0]
        print("AUTO-DISCOVERY found multiple exact 1-200 series:")
        for candidate_parent, _ in candidates:
            print(f"  - {candidate_parent}")
        print(f"AUTO-SELECTED: {parent}")
        return rows

    found_numbers = sorted({n for n, _ in all_txt})
    missing = sorted(set(range(1, 201)) - set(found_numbers))
    sample_paths = [p for _, p in all_txt[:20]]
    raise RuntimeError(
        "Could not find an exact Episode_0001 to Episode_0200 TXT sequence "
        f"anywhere in source repo '{HF_SOURCE_REPO_ID}'. "
        f"Missing episode numbers across all matching TXT files: {missing}. "
        f"Sample matching paths: {sample_paths}"
    )
