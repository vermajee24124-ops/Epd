...
def call(url,key,payload,label,tries=6):
 ...
 for a in range(1,tries+1):
  try:
   ...
   elif r.status_code in (400,401,403,404,413): raise RuntimeError(f"NON-RETRYABLE HTTP {r.status_code}: {r.text[:1000]}")
...
EPISODE {n}\nCURRENT RAW TRANSCRIPT:\n{cur}"""
 return call(GROQ_URL,GROQ_API_KEY,{"model":GROQ_MODEL,"messages":[{"role":"user","content":prompt}],"temperature":0.05,"max_tokens":GROQ_MAX_OUTPUT_TOKENS,"stream":False},f"GROQ EP{n:04d}")
